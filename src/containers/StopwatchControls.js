...
export class StopwatchControls extends Component {
  render(props) {
    const { started,onStartTimer, onStopTimer, onResetTimer, onAddLap,keys, name, currentTask } = this.props;
    console.log(this.props.name);
    if (started) {
      return (
        <div className="stopwatch-controls">
          <Button class="button" style={{backgroundColor: "white", color:"red"}} onClick={() => onStopTimer(convertToCentiSeconds(Date.now()), keys, currentTask)}>
      ...
      return (
        <div className="stopwatch-controls">
          <Button className="button" style={{backgroundColor: "white", color:"green"}}onClick={() => onStartTimer(convertToCentiSeconds(Date.now()),this.props.name)}>
...
const mapDispatchToProps = (dispatch, state) => ({
  ...
  onStopTimer: (change, keys, currentTask) => {
    dispatch(updateStopTimer(change, keys, currentTask)).then(dispatch(resetTimer()));
   },
  // onResetTimer: () => {
  //   dispatch(resetTimer());
  // },
